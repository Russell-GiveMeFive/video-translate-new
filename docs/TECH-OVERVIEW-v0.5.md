...
> 2026-07-01 · 2026-07-13 按当前代码重新同步
>
...
6. ocr-assist        — VLM OCR 实现保留但当前全局 disabled，运行时 skipped 并沿用 ASR 切句
...
- **后处理**：mix-render 对 `accompaniment` 过 `highpass=60, lowpass=12000, acompressor`，兼顾残留抑制与高频空气感

**当前状态**：PyInstaller standalone 的构建脚本、三平台 GitHub Actions 配置和 `electron-builder extraResources` 接入点已完成。运行时优先使用 system demucs，找不到时回退 bundled binary。`binaries/` 被 Git 忽略，因此干净 clone 或正式发版前仍需构建/下载并验证对应平台产物；缺失时才需要 `pip install demucs` 兜底。
...
- **边界平滑**：每个足够长的 range 在回填时自动加 80ms `afade in/out`，降低切换点 pop/click
...
| 限制 | 影响 | 缓解 |
|------|------|------|
| Demucs binary 是 Git 外构建产物 | 干净 clone / 新平台若未准备 binary，且系统无 demucs，会导致 demix skip | 发版前从 workflow 下载对应 artifact，或运行 packaging 脚本构建 |
...
1. **Demucs 分发闭环** — 自动把各平台 workflow artifact 注入正式安装包，并补齐运行时自检
...
- [ ] 发版流水线自动拉取并校验 macOS arm64/x64、Windows x64 Demucs binary
